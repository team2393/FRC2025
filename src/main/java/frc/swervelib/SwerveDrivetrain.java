...
abstract public class SwerveDrivetrain extends SubsystemBase
{
  ...
  */
  ...
  public Command followTrajectory(Trajectory trajectory, double end_angle, boolean require_drivetrain)
  {
    ...
    // Storage for last swerve states
    final AtomicReference<List<SwerveModuleState>> last_states = new AtomicReference<>();
    ...
    Command show_last = new InstantCommand(() ->
    {
      System.out.println("Last trajectory states:");
      for(var state : last_states.get())
        System.out.println(state);
    });
    Command stop = new InstantCommand(this::stop);
    return follower.andThen(show_last).andThen(stop);
  }
}
